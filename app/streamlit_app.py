...
if selected_page == "🏠 Dashboard":
    ...
    tab_single, tab_batch = st.tabs(["📷 Single Slice Image Analysis", "📁 Batch MRI Classification"])
    ...
    with tab_single:
        if best_model is None:
            ...
            mode = st.radio("Choose Input Mode", ["Upload MRI Scan File", "Use Pre-loaded Demo Scans"], horizontal=True, key="single_mode_radio")
            ...
            if img_data is not None:
                ...
                with col2:
                    ...
                    with placeholder.container():
                        ...
                        step1 = st.empty()
                        step1.markdown('<div class="pipeline-step active">📤 Step 1/4: Uploading MRI slice data...</div>', unsafe_allow_html=True)
                        time.sleep(0.3)
                        step1.markdown('<div class="pipeline-step complete">✅ Step 1/4: Uploading MRI slice data complete</div>', unsafe_allow_html=True)
                        
                        step2 = st.empty()
                        step2.markdown('<div class="pipeline-step active">⚙️ Step 2/4: Applying preprocessing pipelines...</div>', unsafe_allow_html=True)
                        time.sleep(0.4)
                        step2.markdown('<div class="pipeline-step complete">✅ Step 2/4: Preprocessing complete (resized to 224x224x3, channels converted, normalized)</div>', unsafe_allow_html=True)
                        
                        step3 = st.empty()
                        step3.markdown('<div class="pipeline-step active">🧠 Step 3/4: Running TensorFlow model inference...</div>', unsafe_allow_html=True)
                        try:
                            probs, inference_time = execute_inference(image_array, best_model)
                            time.sleep(0.3)
                            step3.markdown('<div class="pipeline-step complete">✅ Step 3/4: Inference complete</div>', unsafe_allow_html=True)
                        except Exception as e:
                            app_logger.error(f"Prediction failed for file '{filename}': {e}", exc_info=True)
                            st.error(f"Prediction Failure: {e}")
                            
                        step4 = st.empty()
                        step4.markdown('<div class="pipeline-step active">📄 Step 4/4: Compiling Grad-CAM maps & AI reports...</div>', unsafe_allow_html=True)
                        time.sleep(0.3)
                        step4.markdown('<div class="pipeline-step complete">✅ Step 4/4: Diagnostic reports compiled successfully!</div>', unsafe_allow_html=True)
                    ...
                    if probs is not None:
                        ...
                        pred_class, conf = render_prediction_results(
                            probs,
                            CLASS_LABELS,
                            precision=st.session_state.settings_precision
                        )
                        
                        # Save prediction logs
                        add_to_history(filename, pred_class, conf, inference_time)
                        app_logger.info(f"Successful prediction: File='{filename}', Pred='{pred_class}', Conf={conf:.4f}, Time={inference_time:.3f}s")
    ...
    with tab_batch:
        if not st.session_state.settings_batch:
            st.warning("⚠️ Batch prediction is disabled in Application Settings. Enable it to run multiple predictions.")
            ...
            if uploads:
                ...
                for idx, (img_arr, filename) in enumerate(uploads):
                    try:
                        ...
                        app_logger.error(f"Batch prediction failed for '{filename}': {e}", exc_info=True)
                        batch_results.append({
                            "Filename": filename,
    ...
    with col1:
        ...
        if has_psutil:
            ...
            st.write(f"**System RAM Usage:** {mem_used:.2f} GB / {mem_total:.2f} GB ({mem.percent}%)")
        else:
            st.write("**System RAM Usage:** psutil not installed (N/A)")
            
    with col2:
        st.subheader("Diagnostic Metrics Summary")
        summary_file = Path(config.OUTPUT_DIR) / "evaluation" / "reports" / "evaluation_summary.json"
        ...
        if summary_file.exists():
            ...
            st.info("Evaluation metrics summary not discovered. Run Module 3 pipeline to generate stats.")
