...
def main():
    ...
    error_analysis_data = {"correct": correct_preds[:20], "incorrect": incorrect_preds[:20]}
    ...
    print(
        f"Saved 20 correct & 20 incorrect audit cases to {saved_models_dir / 'error_analysis.json'}"
    )
    ...
    for cls in classes:
        ...
        for idx, row in sample_rows.iterrows():
            ...
            img_tensor = preprocess_single_image(
                img_raw, target_size=config.IMAGE_SIZE, is_bgr=True
            )
            img_tensor_batch = tf.expand_dims(img_tensor, axis=0)
            probs = model.predict(img_tensor_batch, verbose=0)[0]
            pred_idx = np.argmax(probs)
            ...
            confidence = probs[pred_idx]
            print(
                f"  Scan: {Path(img_path).name} -> Actual: {cls.upper()} | Predicted: {pred_class.upper()} ({confidence:.2%})"
            )
...
"""
    ...
    print("=" * 60)
    print(
        f"1. Dataset Statistics: Train={len(metadata.get('train', []))}, Val={len(metadata.get('validation', []))}, Test={len(test_df)}"
    )
    print(
        "2. Changes Made: Added L2 Regularization to Dense layer, added RandomContrast (0.15 factor) to augmentation pipeline, optimized initial lr to 1e-3 (frozen) and fine-tuning lr to 5e-5."
    )
